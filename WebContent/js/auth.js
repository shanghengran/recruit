var Auth = {
	vars: {
		...
		type_group:document.querySelector('.type_group'),
		val_group:document.querySelector('.val_group'),
	...
	forgot(e) {
		...
		setTimeout(() => {
        	Auth.vars.type_group.style.display = 'none';
        	Auth.vars.val_group.style.display = 'none';
	...
	loginback(e) {
        ...
        setTimeout(() => {
            Auth.vars.type_group.style.display = 'block';
        	Auth.vars.val_group.style.display = 'block';
    	}, 600);
	...
	init(option) {
		...
		Auth.vars.lowin_register.querySelector('form').setAttribute('action', option.register_url);
